...
## 0.3 alpha 8 — récolte partagée et atelier (22 septembre 2026)

`BQ_TEST_AUTH=1 BQ_TEST_MONGO=1 npm test` : **64 tests réussis**, aucun échec ni test ignoré. Neuf tests métier couvrent piles/capacité, banque, refus d’équipement des ressources, coûts et niveaux de recette, sac plein avec case libérée, réservation exclusive, interruption, renouvellement, échec de stockage, emplacement des nœuds et sac rempli pendant la récolte. Le parcours WebSocket fait marcher deux personnages vers le même frêne : un seul récolte ; l’autre est refusé. Après deux gisements de fer, fabrication distante refusée, visite de Brann, double commande donnant un seul objet, équipement et reconnexion avec objets/XP/pièces conservés.

`scripts/browser-crafting.js` : deux comptes confirmés via GoTrue/Mailpit dans des contextes Chromium distincts. Le personnage de test récupéré commence avec 77 pièces, sans ressources ; le parcours récolte réellement le bois et les quatre minerais par clic sur les sprites, voit l’indisponibilité sur le deuxième compte, puis fabrique une épée (74 pièces restantes), l’équipe et change/reprend son personnage. XP finale : bûcheron 3, mineur 6, forgeron 10. Le sac montre le compteur de pile et aucun bouton d’équipement sur une ressource. Écrans métier/HUD contrôlés à 390×844, sans débordement horizontal ni erreur JavaScript. Les boutons de recette restent désactivés hors atelier ou avec coûts/niveau insuffisants.

Préparation : serveur normal sur 8086, `MONGODB_DATABASE=bq_qa_crafting`, puis `MONGODB_DATABASE=bq_qa_crafting node scripts/prepare-account-qa.js crafting` ; exécuter `output/browser-crafting-run.js` avec Playwright CLI. Les fixtures et comptes sont distincts des données jouables de 8085. Les captures locales `crafting-*.png` montrent récolte, piles du sac, atelier et mobile. Le contrôle visuel a conduit à densifier le feuillage du frêne et à déplacer Brann hors de la canopée du décor.

Les cinq nouveaux sprites sont générés puis réduits/contrôlés sur leur grille native : nœuds 32×40 et 32×24, ressources 16×16, enclume 32×32, transparence binaire. Les sources et aperçus restent dans `output/crafting-assets/`. La silhouette du frêne a été contrôlée de nouveau dans le jeu après correction.

Limites : six nœuds près du village, deux ressources, trois recettes. Les nœuds se réinitialisent au redémarrage ; possessions et métiers sont persistants. Pas de découpage manuel des piles, d’outil de récolte, d’économie équilibrée, ni de preuve de charge MMO. Google/Apple restent en attente de configuration externe ; classes et percepteurs restent à réaliser.

Le passage d’une session initialement tactile à une fenêtre desktop a aussi reproduit un sprite de survol absent : les silhouettes n’étaient pas préparées au démarrage mobile. `Entity.setHighlight` conserve désormais le sprite normal si la silhouette manque, et reste idempotent. Test de régression et parcours navigateur rejoué après correction. Le placement des noms de ressources utilise le sprite normal, indépendamment du survol.
